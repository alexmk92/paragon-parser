...
Queue.prototype.failed = function(replay) {
    this.workerDone(replay).then(function() {
        ...
        var query = 'UPDATE queue SET reserved_by=null, completed = false, checkpointTime = 0, attempts = attempts + 1, priority = 2, scheduled = DATE_ADD(NOW(), INTERVAL 2 MINUTE), reserved = false WHERE replayId = "' + replay.replayId + '"';
...
Queue.prototype.schedule = function(replay, ms) {
    this.workerDone(replay).then(function() {
        ...
        var query = 'UPDATE queue SET reserved_by=null, reserved = false, scheduled = DATE_ADD(NOW(), INTERVAL 1 MINUTE), priority=3, checkpointTime=' + replay.replayJSON.latestCheckpointTime + ' WHERE replayId="' + replay.replayId + '"';
...
Queue.prototype.removeDeadReplay = function(replay) {
    this.workerDone(replay).then(function() {
        ...
        var conn = new Connection();
        var query = 'UPDATE queue SET reserved_by=null, reserved=false, completed=true, completed_at=NOW() WHERE replayId="' + replay.replayId + '"';
        conn.query(query, function() {
...
Queue.disposeOfLockedReservedEvents = function(processId, callback) {
    ...
    var conn = new Connection();
    var query = 'UPDATE queue SET reserved=false, reserved_by=null WHERE reserved_by="' + processId + '"';
    conn.query(query, function() {
