...
Queue.prototype.getNextJob = function() {
    ...
    if(!fetching && this.workers.length < this.maxWorkers) {
        ...
        var selectQuery = 'SELECT * FROM queue WHERE completed = false AND reserved = false AND scheduled <= NOW() ORDER BY created_at ASC LIMIT 1 FOR UPDATE';
    ...
    return new Promise(function(resolve, reject) {
        if(!removing) {
            ...
            if(index > -1) {
                ...
                console.log('[QUEUE] Worker '.cyan + (index+1) + ' was successfully removed from the queue, there are '.cyan + this.workers.length + '/'.cyan + this.maxWorkers + ' workers running'.cyan);
            }
            resolve();
        } else {
...
Queue.prototype.uploadFile = function(replay, callback) {
    if(replay.replayJSON !== null) {
        try {
            ...
            this.failed(replay);
            console.log('[MONGO ERROR] in Queue.js when uploading relay: '.red + replay.replayId + '.  Error: '.red, e);
